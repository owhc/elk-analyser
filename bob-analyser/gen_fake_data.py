# Created: 2026-08-26 16:19:44 +0800
"""
gen_fake_data.py
────────────────
產生 WAS / MQ / DB2 模擬日誌並放置至共享 Volume，供 elk-analyser 主動分析。
...
使用方式：
  ...
  # 3. 調整時間範圍
  podman exec elk-analyser python gen_fake_data.py --from "2025-01-15 08:00" --to "2025-01-15 09:00"

包含四個真實場景：
  場景 A (TRC-DB2-001)：DB2 連線池耗盡 → WAS FATAL → MQ 訊息堆積
  場景 B (TRC-WAS-002)：WAS JVM 記憶體洩漏 → GC 過載 → 服務降級
  場景 C (TRC-MQ-003)：MQ SSL 憑證過期 → 連線斷線 → 訊息投遞失敗
  場景 D (TRC-DB2-004)：DB2 磁碟滿 → 交易日誌滿 → 全服務阻塞
"""
...
def _build_logs(base_time: datetime) -> list[dict]:
    ...
    return [
        # ── 場景 A：DB2 連線池耗盡 → WAS FATAL → MQ 訊息堆積 ──────────
        {"@timestamp": ts(0,5),  "log.level": "ERROR",    "service.name": "db2-server",
         "host.name": "db2-prod-01",  "trace.id": "TRC-DB2-001",
         "message": "SQLCODE=-1218 SQLSTATE=57011: DB2 connection pool exhausted. Max connections=200 reached. Waiting queue=145."},
        {"@timestamp": ts(0,20), "log.level": "ERROR",    "service.name": "db2-server",
         "host.name": "db2-prod-01",  "trace.id": "TRC-DB2-001",
         "message": "DBI1039E: Database connection refused. Active connections=200/200. New requests are being rejected."},
        {"@timestamp": ts(0,35), "log.level": "CRITICAL", "service.name": "db2-server",
         "host.name": "db2-prod-01",  "trace.id": "TRC-DB2-001",
         "message": "DB2 SEVERE: Deadlock detected on table ORDERS. SQLCODE=-911. Rolling back transaction TXN-4821."},
        {"@timestamp": ts(1,10), "log.level": "ERROR",    "service.name": "was-order-svc",
         "host.name": "was-prod-01",  "trace.id": "TRC-DB2-001",
        ...
        {"@timestamp": ts(1,45), "log.level": "ERROR",    "service.name": "was-order-svc",
         "host.name": "was-prod-01",  "trace.id": "TRC-DB2-001",
         "message": "TRAS0017I: Transaction rolled back due to FFDC: java.sql.SQLException: DB2 SQL Error: SQLCODE=-1218, SQLSTATE=57011. OrderID=ORD-20250115-8821"},
        {"@timestamp": ts(2,5),  "log.level": "ERROR",    "service.name": "mq-manager",
         "host.name": "mq-prod-01",   "trace.id": "TRC-DB2-001",
        ...
        {"@timestamp": ts(12,15), "log.level": "ERROR",    "service.name": "db2-server",
         "host.name": "db2-prod-01",  "trace.id": "TRC-WAS-002",
         "message": "SQL20542N: Long-running query detected. Query running for 285 seconds. APPNAME=InventoryApp. Statement: SELECT * FROM INVENTORY WHERE STATUS='ACTIVE' (rows scanned: 2.3M)"},
        {"@timestamp": ts(12,45), "log.level": "FATAL",    "service.name": "was-inventory-svc",
         "host.name": "was-prod-02",  "trace.id": "TRC-WAS-002",
         "message": "JVMXE010: JVM requested operating system signal. Heap dump generated: /tmp/heapdump.20250115.120000.phd. Process will restart."},
        ...
        {"@timestamp": ts(20,0),  "log.level": "ERROR",    "service.name": "mq-manager",
         ...
         "message": "AMQ9641E: Remote channel 'REMOTE.PAYMENT.CHANNEL' SSL/TLS handshake failed: Certificate expired at 2025-01-15T00:00:00Z. CN=mq-prod-02.example.com"},
        ...
        {"@timestamp": ts(22,0),  "log.level": "ERROR",    "service.name": "db2-server",
         "host.name": "db2-prod-01",  "trace.id": "TRC-MQ-003",
         "message": "SQL0911N: Reason code 68. PAYMENT_TRANSACTIONS table deadlock. Uncommitted transactions from was-payment-svc piling up. Lock wait timeout exceeded."},
        ...
        # ── 場景 D：DB2 磁碟滿 → 交易日誌滿 → 全服務阻塞 ──────────────
        {"@timestamp": ts(35,0),  "log.level": "ERROR",    "service.name": "db2-server",
         "host.name": "db2-prod-01",  "trace.id": "TRC-DB2-004",
         "message": "ADM1823E: The active log is full and is held by application handle=182. Log path: /db2log/NODE0000/LOGSTREAM0000. Available log space=0KB."},
        {"@timestamp": ts(35,20), "log.level": "CRITICAL", "service.name": "db2-server",
         "host.name": "db2-prod-01",  "trace.id": "TRC-DB2-004",
         "message": "SQL0964C: The transaction log for the database is full. SQLSTATE=57011. All write transactions are being rolled back. Read-only access only."},
        {"@timestamp": ts(35,45), "log.level": "ERROR",    "service.name": "was-order-svc",
         "host.name": "was-prod-01",  "trace.id": "TRC-DB2-004",
         "message": "TRAS0017I: FFDC Exception chain: SQLCODE=-964 [DB2 log full] -> ResourceException [connection pool] -> EJBException [OrderService.createOrder()]. 8823 orders failed in last 5 minutes."},
        {"@timestamp": ts(36,10), "log.level": "ERROR",    "service.name": "mq-manager",
         "host.name": "mq-prod-01",   "trace.id": "TRC-DB2-004",
        ...
        {"@timestamp": ts(36,30), "log.level": "FATAL",    "service.name": "db2-server",
         "host.name": "db2-prod-01",  "trace.id": "TRC-DB2-004",
         "message": "DB2 DATABASE IS SUSPENDED: All applications are in a wait state. Affected databases: PRODDB, ORDERDB, AUDITDB"},
...
@click.command()
@click.option("--from", "from_time", default="2025-01-15 08:00",
              help="日誌基準開始時間 (YYYY-MM-DD HH:MM)")
@click.option("--to",   "to_time",   default="2025-01-15 09:00",
...
def main(from_time: str, to_time: str, no_trigger: bool, volume_dir: str):
    ...
    logger.info("已產生 %d 筆模擬日誌（WAS/MQ/DB2，4 個場景）", len(logs))

    # 寫入共享 Volume（供 bob_bridge 讀取）
    vol_dir = Path(volume_dir)
    vol_dir.mkdir(parents=True, exist_ok=True)
    vol_path = vol_dir / "was_mq_db2_fake.json"
